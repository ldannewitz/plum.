...
RSpec.describe Event, type: :model do
  ...
  describe '#generate_invoices' do
    let!(:expense) { Expense.create!(event: event, spender_id: david.id, description: "Redbull", amount: 6.20) }
    ...
    # end
    #
    # it 'generates a 0 invoice if a user has no balance' do
    #   # fake_invoice_api = instance_double("InvoiceApi")
    #   # InvoiceApi.any_instance
    #   # stub(InvoiceApi).new { fake_invoice_api }
    #   event.update(total: 12.4)
    #   event.generate_invoices
    #   unit_price = david.bills.last.amount * -1
    #
    #   byebug
    #   expect(david.bills.count).to equal(1)
    #   allow_any_instance_of(InvoiceApi).to receive(:create_invoice).with(david.email, event.name, unit_price)
    # end
  end
